...
public class OperationTimeout {
     ...
     */
    public <T> T executeWithTimeout(Callable<T> operation, String operationName) throws TimeoutException, InterruptedException {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                logger.warning(operationName + " timed out on attempt " + attempt + " (timeout: " + timeoutMs + "ms)");
                if (attempt == maxRetries) {
                    executor.shutdownNow();
                    throw new TimeoutException("Operation '" + operationName + "' failed after " + maxRetries + " attempts");
                }
     ...
     */
    public void executeWithTimeout(Runnable operation, String operationName) throws TimeoutException, InterruptedException {
        executeWithTimeout(() -> {
            operation.run();
            return null;
        }, operationName);
